...
ColumnFamily.prototype.enumerate = function() {
    ...
    if (!callback) {
        throw "Callback function must be passed in as " +
                "function to enumerate. Enumerate() accepts 3 " +
                "functions which must be given in the following order: " +
                "enumerate(callback, done, error_handler)."
    }
    ...
    get_range_slices(
      ...
      , function get_range_slices_callback(error, rows) {
            ...
            var row_callback = function row_callback(error, i) {
                if (error) {
                    return error_handler(error)
                }
                ...
                if (typeof row === 'undefined') {
                    ...
                    // TODO new error type
                    return error_handler('Undefined row key in Column Family')
                }
